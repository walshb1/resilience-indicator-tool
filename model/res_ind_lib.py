...
def compute_resiliences(df_in=None, kind="exante", fa_ratios=None, is_relative_to_local=True, verbose_output=True):
    ...
    if fa_ratios is not None:
        ...
        fa_ratios = interpolate_faratios(
            fa_ratios, df_in.protection.unique().tolist())

        # builds a dataframe multi-indexed by return period  (country, (var,
        # rp))
        ...
        # Reshapes into ((country,rp), vars) formally using only country as
        # index
    ...
    vp_shew, vr_shew = unpack_v(
        v_shew, pv, df.faref, df.peref, ph, df.share1_ref)
    ...
    df["institutional_exposure"] = (
        (1 - df["axhealth"]) + (1 - df["plgp"]) + df["unemp"]) / 3
    ...
    delta_W, dK, foo, foo = calc_delta_welfare(
        ph, fap, far, vp_shew, vr_shew, vs_shew, cp, cr, tot_p, tot_r, mu, share_shareable, gamma, rho, elast)

    if verbose_output:
        # Assuming no scale up
        dW_noupscale, foo, foo, foo = calc_delta_welfare(ph, fap, far, vp_shew, vr_shew, vs_shew, cp, cr, df[
                                                         "social_p"], df["social_r"], mu, share_shareable, gamma, rho, elast)

        # Assuming no transfers at all
        dW_no_transfers, foo, foo, foo = calc_delta_welfare(
            ph, fap, far, vp_shew, vr_shew, vs_shew, cp, cr, 0, 0, mu, share_shareable, gamma, rho, elast)
    ...
    individual_exposure = (
        ...
        (1 - ph) * far * (1 - df["axfin_r"]) * THETA(cr -
                                                     trap_treshold, cr * (1 - tot_r) * vr_shew, df["H"])
    )
    ...
    df["dW_destitution"] = (tot_exposure * (welf(df["gdp_pc_pp"] / rho, elast) - welf(
        df["gdp_pc_pp"] / rho - df["dC_destitution"], elast))) / df["protection"]
    ...
    if fa_ratios is not None:
        ...
        proba = pd.Series(np.diff(np.append(1 / fa_ratios.columns.values, 0)
                                  [::-1])[::-1], index=fa_ratios.columns)  # removes 0 from the rps
        ...
        f = df.mul(proba_serie, axis=0).sum(level="country").div(
            proba_serie.sum(level="country"), axis=0)
    ...
    if is_relative_to_local:
        wprime = (welf(df["gdp_pc_pp"] / rho + h, df["income_elast"]) -
                  welf(df["gdp_pc_pp"] / rho - h, df["income_elast"])) / (2 * h)
    else:
        wprime = (welf(df["gdp_pc_world"] / rho + h, df["income_elast"]) -
                  welf(df["gdp_pc_world"] / rho - h, df["income_elast"])) / (2 * h)
    ...
    if verbose_output:
        ...
        df["total_equivalent_cost_of_destitution"] = df["total_equivalent_cost"] * \
            df["dW_destitution"] / (df["dW_destitution"] + df["delta_W"])

        df["total_equivalent_cost_no_destitution"] = df[
            "total_equivalent_cost"] * df["delta_W"] / (df["dW_destitution"] + df["delta_W"])
...
# TODO : AVOID THIS "LOOP" AND RELY ON ASSIGN outside the main function to
# compute this
...
def calc_delta_welfare(ph, fap, far, vp, vr, v_shared, cp, cr, la_p, la_r, mu, sh_sh, gamma, rho, elast):
    ...
    # v_shared does not change with v_rich so pv changes only vulnerabilities
...
def unpack_v(v, pv, fa, pe, ph, share1):
    # poor and non poor vulnerability from aggregate vulnerability,  exposure
    # and biases
...
def interpolate_faratios(fa_ratios, protection_list):
    ...
    # extrapolates linear towards the 0 return period exposure  (this creates
    # negative exposure that is tackled after interp) (mind the 0 rp when
    # computing probas)
    ...
    # add new, interpolated values for fa, assuming constant exposure on the
    # right
    x = fa_ratios_rps.columns.values
    y = fa_ratios_rps.values
    fa_ratios_rps = pd.concat(
        [pd.DataFrame(interp1d(x, y, bounds_error=False)(all_rps), index=fa_ratios_rps.index, columns=all_rps)], axis=1).sort_index(axis=1).clip(lower=0).fillna(method="pad", axis=1)
...
def compute_derivative(df_original, score_card_set, deriv_set, fa_ratios=None, **kwargs):
    ...
    der = pd.DataFrame(index=df_original.dropna().index, columns=pd.MultiIndex.from_tuples(
        headr, names=["var", "out"])).sortlevel(0, axis=1)
    # der.columns
    for var in deriv_set:
        ...
        # this reads (all countries, (this var, all output))  = (all countries,
        # all output)
        der[var] = (fxh - fx) / (h)
    ...
    der_risk_rel = pd.DataFrame(
        (der["risk"]["axfin_p"] / der["resilience"]["axfin_p"]))
    ...
    derivatives = pd.concat([der, der_risk_rel, der_risk_vshew], axis=1).sortlevel(
        0, axis=1)  # (countries, (outputs, inputs))
